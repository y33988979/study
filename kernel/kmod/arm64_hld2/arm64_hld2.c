...
static u64 (*_arch_irq_stat_cpu)(unsigned int cpu);
/*
...
static void hld_watchdog_check(void)
{
	...
	u64 irqs_sum = 0, arch_irq_sum = 0;

	now = get_timestamp();

	for_each_online_cpu(cpu) {
		/* get percpu total hardirq count */
		irqs_sum = kstat_cpu_irqs_sum(cpu);
		if (_arch_irq_stat_cpu)
			arch_irq_sum = _arch_irq_stat_cpu(cpu);
		...
		if (unlikely(duration)) {
			...
			pr_warn("CPU##%d: irq off %ds, now: %lus, last_touch: %lus, irqs: %llu, arch_irqs: %llu\n", 
				cpu, duration, now, touch_ts, irqs_sum, arch_irq_sum);

			/*
  			* smp_call_func_signle may be blocked on csd_lock_wait
 			* it waiting flag CSD_FLAG_LOCK is clear. 
 			* 
 			* We need to be very careful, so we don't send IPI..
 			*
 			* TODO ...
 			*/
			//smp_call_function_single(cpu ,func, 0);
...
static __init int hld_callback_init(void)
{
	_arch_irq_stat_cpu = kallsyms_lookup_name("smp_irq_stat_cpu");
	if (!_arch_irq_stat_cpu)
		pr_warn("WARNING: symbol 'smp_irq_stat_cpu' not found\n");

	return !!_arch_irq_stat_cpu;
}
